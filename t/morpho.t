...
ok(my $morpho = Morpho::load('t/data/test.dict'));

sub analyze {
  my ($word, $results, $found) = @_;
  $found = $Morpho::NO_GUESSER unless $found;

  my $lemmas = TaggedLemmas->new();
  is($morpho->analyze($word, $Morpho::NO_GUESSER, $lemmas), $found);
...
sub generate {
  ...
  my $lemmas = TaggedLemmasForms->new();
  is($morpho->generate($word, $wildcard, $Morpho::NO_GUESSER, $lemmas), $Morpho::NO_GUESSER);
